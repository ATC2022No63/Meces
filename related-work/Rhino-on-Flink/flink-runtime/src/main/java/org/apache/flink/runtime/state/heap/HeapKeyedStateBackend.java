...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
    ...
    // ------------------------------------------------------------------------
    @Override
    public Map<String, StatePartition> getIncrementalState(boolean isFull) throws IOException {
        Map<String, StatePartition> states = new HashMap<>();
        for (Map.Entry<String, StateTable<K, ?, ?>> entry : registeredKVStates.entrySet()) {
            LOG.info("incremental snapshot done:{}", entry.getKey());
            states.put(entry.getKey(), entry.getValue().getSnapshot(isFull));
        }
        return states;
    }

    public void changeKeyGroupRange(int maxparallelism, int parallelism, int newIndex) {
        for (StateTable<K, ?, ?> table : registeredKVStates.values()) {
            table.changeKeyGroupRange(maxparallelism, parallelism, newIndex);
        }
    }
    ...
    public void applySnapshot(byte[] bytes, boolean full) throws IOException {
        ...
        for (int i = 0; i < nstate; i++) {
            String tableName = ss.deserialize(dd);
            int length = dd.readInt();
            byte[] buffer = new byte[length];
            dd.read(buffer);
            assert registeredKVStates.containsKey(tableName);
            StateTable st = registeredKVStates.get(tableName);
            st.applySnapshot(buffer, full);
        }
