...
import org.apache.flink.runtime.state.replication.StatePartition;
...
public abstract class StateTable<K, N, S>
    ...
    protected Map<N, Set<K>> dirtyKeys = new ConcurrentHashMap<>();
    ...
    public StatePartition getSnapshot(boolean isFull) throws IOException {
        ...
        Map<N, Map<K, S>> state = new HashMap<>();
        Map<N, Set<K>> dirty = this.dirtyKeys;
        this.dirtyKeys = new ConcurrentHashMap<>();
        for (Map.Entry<N, Set<K>> entry : dirty.entrySet()) {
            N ns = entry.getKey();
            for (K k : entry.getValue()) {
                S value = stateSerializer.createInstance();
                state.computeIfAbsent(ns, n -> new HashMap<>()).put(k, value);
            }
        }
        if (isFull)
            for (StateEntry<K, N, S> entry : this) {
                state.computeIfAbsent(entry.getNamespace(), n -> new HashMap<>()).put(entry.getKey(), entry.getState());
            }
        return new StatePartition(state, namespaceSerializer, keySerializer, stateSerializer);

    }

    public void changeKeyGroupRange(int maxparallelism, int parallelism, int newIndex) {
        int oldstart = keyContext.getKeyGroupRange().getStartKeyGroup();
        int oldend = keyContext.getKeyGroupRange().getEndKeyGroup();
        int oldsize = this.keyGroupedStateMaps.length;
        KeyGroupRange newRange = KeyGroupRangeAssignment.computeKeyGroupRangeForOperatorIndex(maxparallelism, parallelism, newIndex);
        int newstart = newRange.getStartKeyGroup();
        int newend = newRange.getEndKeyGroup();
        int newSize = newRange.getNumberOfKeyGroups();
        @SuppressWarnings("unchecked")
        StateMap<K, N, S>[] state =
                (StateMap<K, N, S>[])
                        new StateMap[newSize];

        if (newSize > oldsize) {
            for (int i = 0; i < oldsize; i++) {
                state[i] = this.keyGroupedStateMaps[i];
            }
        ...
        this.keyContext.setKG(newRange);
        this.keyGroupedStateMaps = state;
        this.keyGroupOffset = newRange.getStartKeyGroup();
        log.info("prev kg start:{} end:{},new kg start:{},end：{} ", oldstart, oldend, newstart, newend);
    }

    public void applySnapshot(byte[] bytes, boolean full) throws IOException {
        DataInputDeserializer dd = new DataInputDeserializer(bytes);
        TypeSerializer<N> namespaceSerializer = getNamespaceSerializer();
        TypeSerializer<K> keySerializer = getKeySerializer();
        TypeSerializer<S> stateSerializer = getStateSerializer();
        if (full) {
            log.info("destroy previous state");
            for (int i = 0; i < this.keyGroupedStateMaps.length; i++) {
                this.keyGroupedStateMaps[i] = createStateMap();
            }
        }
        int nentry = dd.readInt();
        log.info("table applies {} entries", nentry);
        for (int i = 0; i < nentry; i++) {
            N ns = namespaceSerializer.deserialize(dd);
            K key = keySerializer.deserialize(dd);
            S value = stateSerializer.deserialize(dd);
            int keyGroup =
                    KeyGroupRangeAssignment.assignToKeyGroup(key, keyContext.getNumberOfKeyGroups());
            put(key, keyGroup, ns, value);
        }
    ...
    private S get(K key, int keyGroupIndex, N namespace) {
        ...
        if (stateMap == null) {
            return getStateSerializer().createInstance();
        }
    ...
    StateMap<K, N, S> getMapForKeyGroup(int keyGroupIndex) {
        ...
        if (pos >= 0 && pos < keyGroupedStateMaps.length) {
            ...
            if (!error) {
                log.warn("error index:{},kg:{},offset:{}", pos, keyGroupIndex, keyGroupOffset);
                error = true;
            }
    ...
    public void put(K key, int keyGroup, N namespace, S state) {
        ...
        dirtyKeys.computeIfAbsent(namespace, n -> new HashSet<>()).add(key);
    }
