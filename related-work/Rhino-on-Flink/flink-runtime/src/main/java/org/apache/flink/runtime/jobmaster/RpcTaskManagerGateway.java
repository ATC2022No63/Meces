...
//        taskExecutorGateway.startReplication();
    ...
    @Override
    public void prepareForRescaling(rescalePlan plan) {
        CompletableFuture<Acknowledge> ack = taskExecutorGateway.prepareForRescaling(plan);
        try {
            ack.get();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public void triggerSignal(ExecutionAttemptID eid) {
        taskExecutorGateway.triggerSignal(eid);
    }

    @Override
    public CompletableFuture<Void> startReplication(int epoch) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        CompletableFuture.runAsync(() -> {
            taskExecutorGateway.startReplication(epoch);
