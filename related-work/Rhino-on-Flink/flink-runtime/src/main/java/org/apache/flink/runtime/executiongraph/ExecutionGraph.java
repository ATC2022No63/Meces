...
import org.apache.flink.runtime.state.replication.ReplicationCoordinator;
...
public class ExecutionGraph implements AccessExecutionGraph {
    ...
    public void enableCheckpointing(
        ...
        this.rescaleCoordinator = new rescaleCoordinator(tasksToTrigger);
    ...
    public void enableNgScheduling(final InternalFailuresListener internalTaskFailuresListener) {
        ...
        checkState(
                ...
                "enableNgScheduling can be only called once");
    ...
    public void scheduleForExecution() throws JobException {
        ...
        if (transitionState(JobStatus.CREATED, JobStatus.RUNNING)) {
            ...
            final CompletableFuture<Void> newSchedulingFuture =
                    SchedulingUtils.schedule(scheduleMode, getAllExecutionVertices(), this);
    ...
    public void failGlobal(Throwable t) {
        ...
        while (true) {
            ...
            if (current == JobStatus.FAILING
                ...
                // we build a future that is complete once all vertices have reached a terminal
                // state
                final ConjunctFuture<Void> allTerminal = cancelVerticesAsync();
                                ...
                                (Void ignored, Throwable throwable) -> {
                                    if (throwable != null) {
                                        transitionState(
                                                ...
                                                JobStatus.FAILED,
                                                new FlinkException(
                                                        "Could not cancel all execution job vertices properly.",
                                                        throwable));
                                    } else {
                                        allVerticesInTerminalState(globalVersionForRestart);
                                    }
    ...
    public void restart(long expectedGlobalVersion) {
        ...
        // TODO remove the catch block if we align the schematics to not fail global within the
        // restarter.
        catch (Throwable t) {
    ...
    private boolean tryRestartOrFail(long globalModVersionForRestart) {
        ...
        if (currentState == JobStatus.FAILING || currentState == JobStatus.RESTARTING) {
            ...
            final boolean isFailureCauseAllowingRestart =
            ...
            boolean isRestartable =
                    isFailureCauseAllowingRestart && isRestartStrategyAllowingRestart;

            if (isRestartable && transitionState(currentState, JobStatus.RESTARTING)) {
                LOG.info("Restarting the job {} ({}).", getJobName(), getJobID());

                RestartCallback restarter =
                        new ExecutionGraphRestartCallback(this, globalModVersionForRestart);
                FutureUtils.assertNoException(
                        restartStrategy
                ...
                LOG.info(
                        ...
                        getJobName(),
                        getJobID(),
                        StringUtils.concatenateWithAnd(cause1, cause2),
                        failureCause);
                onTerminalState(JobStatus.FAILED);

                return true;
            } else {
                // we must have changed the state concurrently, thus we cannot complete this
                // operation
                return false;
            }
        } else {
     ...
     *                    updated
     * @throws ExecutionGraphException if the schedule or update consumers operation could not be
     *                                 executed
     */
    ...
    void notifyExecutionChange(
        ...
        if (newExecutionState == ExecutionState.FAILED) {
            final Throwable ex =
                    error != null ? error : new FlinkException("Unknown Error (missing cause)");
    ...
    public void triggerRescaling() {
        List<ExecutionVertex> vertices = new ArrayList<>();
        for (ExecutionJobVertex value : tasks.values()) {
            for (ExecutionVertex taskVertex : value.getTaskVertices()) {
                vertices.add(taskVertex);
            }
        }

        rescalePlan newPlan = getNewPlan();
        rescaleCoordinator.prepareRescaling(vertices, newPlan);
    }

    public rescalePlan getNewPlan() {
        Map<JobVertexID, Map<Integer, Integer>> plan = new HashMap<>();
        for (ExecutionJobVertex value : tasks.values()) {
            int p = value.getParallelism();
            if (p == 1)
                continue;
            JobVertexID jvid = value.getJobVertexId();
            Map<Integer, Integer> shuffle = new HashMap<>();
            for (int i = 0; i < p; i++) {
                shuffle.put(i, (i + 1) % p);
            }
            plan.put(jvid, shuffle);
        }
        return new rescalePlan(plan);
    }

    public void triggerSignal() {
        assert rescaleCoordinator != null : "null rescale coordinator";
        rescaleCoordinator.triggerSignal();
    }

    public synchronized void enableReplication() {
        if (this.replicationCoordinator != null)
            return;
        Set<ExecutionVertex> executionVertices = new HashSet<>();
        for (ExecutionJobVertex vertex : tasks.values()) {
            executionVertices.addAll(Arrays.asList(vertex.getTaskVertices()));
        }
        this.replicationCoordinator = new ReplicationCoordinator(executionVertices);
    }
    ...
    public void disableReplication(){replicationCoordinator.disable();}
}
