...
import org.apache.flink.runtime.state.replication.OperatorStatePartition;
...
public class MemoryStateBackend extends AbstractFileStateBackend
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(MemoryStateBackend.class);
    ...
    private Map<StateDescriptor, KeyedStateBackend> keyedStateBackends = new HashMap<>();
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(
        ...
        HeapKeyedStateBackend hkbe = new HeapKeyedStateBackendBuilder<>(
                kvStateRegistry,
                keySerializer,
                env.getUserCodeClassLoader().asClassLoader(),
                numberOfKeyGroups,
                ...
                cancelStreamRegistry)
                .build();
        StateDescriptor info = new StateDescriptor(
                jobID,
                env.getJobVertexId(),
                env.getTaskInfo().getIndexOfThisSubtask(),
                keyGroupRange);
        checkNotNull(hkbe, "null hkbe");
        this.keyedStateBackends.put(info, hkbe);
        LOG.info("hkbe init:jvid:{}", env.getJobVertexId());
        return hkbe;
    }
    ...
    @Override
    public Map<StateDescriptor, OperatorStatePartition> getSnapshot(boolean isFull) throws IOException, FlinkException {
        checkNotNull(this.keyedStateBackends);
        Map<StateDescriptor, OperatorStatePartition> states = new HashMap<>();

        try {
            for (Map.Entry<StateDescriptor, KeyedStateBackend> entry : keyedStateBackends
                    .entrySet()) {
                StateDescriptor info = entry.getKey();
                LOG.info("snapshot: jid:{},jvid:{},subtask:{}", info.jobID, info.jobVertexID, info.subtaskIndex);
                states.put(info, new OperatorStatePartition(entry.getValue().getIncrementalState(isFull)));
            }
    ...
    @Override
    public Set<StateDescriptor> getStateDescriptor() throws FlinkException, IOException {
        return this.keyedStateBackends.keySet();
    }

    @Override
    public void changeKeyGroupRange(int maxparallelism, int parallelism, int newIndex) throws FlinkException, IOException {
        for (KeyedStateBackend backend : keyedStateBackends.values()) {
            backend.changeKeyGroupRange(maxparallelism, parallelism, newIndex);
        }
    }

    @Override
    public void applySnapshot(byte[] bytes, boolean full) throws FlinkException, IOException {
        DataInputDeserializer dd = new DataInputDeserializer(bytes);
        int nstate = dd.readInt();
        LOG.info("binary has {} state", nstate);
        for (int i = 0; i < nstate; i++) {
            byte[] metaHeader = new byte[StateDescriptor.getMetaInfoSize()];
            dd.read(metaHeader);
            StateDescriptor meta = new StateDescriptor(metaHeader);
            int length = dd.readInt();
            LOG.info("jobid:{},jvid:{},index:{},len:{}", meta.jobID, meta.jobVertexID, meta.subtaskIndex, length);
            byte[] buffer = new byte[length];
            dd.read(buffer);
            for (Map.Entry<StateDescriptor, KeyedStateBackend> entry : keyedStateBackends.entrySet()) {
                StateDescriptor descriptor = entry.getKey();
                if (meta.jobID.equals(descriptor.jobID) && meta.jobVertexID.equals(descriptor.jobVertexID))
                    entry.getValue().applySnapshot(buffer, full);
            }
        }
    ...
    @Override
    public void applyIncremental(byte[] bytes, int newIndex) throws FlinkException, IOException {
        DataInputDeserializer dd = new DataInputDeserializer(bytes);
        while (dd.available() > 0) {
            ...
            for (int i = 0; i < nstate; i++) {
                ...
                StateDescriptor meta = new StateDescriptor(metaHeader);
                int length = dd.readInt();
                if (meta.subtaskIndex == newIndex) {
                    byte[] buffer = new byte[length];
                    dd.read(buffer);
                    for (Map.Entry<StateDescriptor, KeyedStateBackend> entry : keyedStateBackends.entrySet()) {
                        StateDescriptor descriptor = entry.getKey();
                        if (meta.jobID.equals(descriptor.jobID) && meta.jobVertexID.equals(descriptor.jobVertexID))
                            entry.getValue().applySnapshot(buffer, false);
                    }
                } else {
                    dd.skipBytes(length);
                }
