...
public class SingleCheckpointBarrierHandler extends CheckpointBarrierHandler {
    ...
    public void processEndOfPartition() throws IOException {
        ...
        if (isCheckpointPending()) {
            ...
            CheckpointException exception =
                    new CheckpointException(CHECKPOINT_DECLINED_INPUT_END_OF_STREAM);
    ...
    private void cancelSubsumedCheckpoint(long barrierId) throws IOException {
        CheckpointException exception =
                new CheckpointException("Barrier id: " + barrierId, CHECKPOINT_DECLINED_SUBSUMED);
    ...
    public String toString() {
        return String.format(
                "aligned %s: current checkpoint: %d, current barriers: %d, open channels: %d",
                taskName, currentCheckpointId, numBarriersReceived, numOpenChannels);
    }

    @Override
    public void freezeChannel(InputChannelInfo info) {
        if (!(controller instanceof AlignedController)) {
            LOG.warn("not AlignedController");
            return;
        }
        AlignedController ac = (AlignedController) controller;
        LOG.info("freeze channel {}", info.toString());
        boolean aligned = controller.freezeChannel(info);
        try {
            if (aligned) {
                this.toNotifyOnCheckpoint.aligned();
                ac.resumeConsumption();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void resumeAll() {
        ...
        try {
            ((AlignedController) controller).resumeConsumption();
            LOG.info("resume consumption!");
        } catch (Exception e) {
            e.printStackTrace();
            LOG.warn("resume fail");
        }
    }
}
