...
import org.apache.flink.runtime.state.replication.BinaryState;
import org.apache.flink.runtime.state.replication.ProcessLevelConf;
import org.apache.flink.runtime.state.replication.ReplicationManager;
import org.apache.flink.runtime.state.replication.StateDescriptor;
import org.apache.flink.runtime.taskexecutor.rescale.rescaleSignal;
import org.apache.flink.runtime.state.ttl.TtlTimeProvider;
import org.apache.flink.runtime.taskexecutor.rescale.stateLocation;
...
import org.apache.flink.runtime.taskexecutor.rescale.rescaleManager;
...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable
    ...
    @Override
    public BinaryState getSnapshot(boolean isFull) throws IOException, FlinkException {
        if (this.rescaleManager == null) {
            this.rescaleManager = new rescaleManager(this);
            this.rescaleManager.init();
        }
        if (stateBackend == null)
            return new BinaryState(new HashMap<>());
        return new BinaryState(stateBackend.getSnapshot(isFull));
    }

    @Override
    public Set<StateDescriptor> getStateDescriptors() throws FlinkException, IOException {
        return stateBackend.getStateDescriptor();
    }
    ...
    public void broadcastSignal() {
        try {
            LOG.info("broadcast signal downstream");
            if (recordWriter instanceof NonRecordWriter)
                return;
            setNewRouting();
            recordWriter.getRecordWriter(0).broadcastEvent(new rescaleSignal(), true);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void aligned() {
        try {
            int nstate = getStateDescriptors().size();
            LOG.info("aligned index{}", getIndexInSubtaskGroup());
            if (nstate != 0) {
                LOG.info("aligned,start state migration");
                int futureIndex = getFutureIndex();
                StateDescriptor owned = getStateDescriptors().iterator().next();
                StateDescriptor wanted = new StateDescriptor(owned.jobID, owned.jobVertexID, futureIndex, null);
                List<CompletableFuture<Void>> futures = this.rescaleManager.pubAndPost(owned);
                stateLocation location = this.rescaleManager.sub(wanted);
                byte[] fetched = this.rescaleManager.fetch(wanted, location.host, location.port);
                LOG.info("fetched state for index:{},size:{}", futureIndex, fetched.length);
                changeKeyGroupRange(futureIndex);
                boolean shouldFull = shouldFull();
                if (!shouldFull) {
                    List<byte[]> incrementalSnapshot = replicationManager.getIncrementalSnapshot(null);
                    for (byte[] bytes : incrementalSnapshot) {
                        applyIncremental(bytes, futureIndex);
                    }
                }
                applySnapshot(fetched, shouldFull);
            }
            broadcastSignal();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void setNewRouting() throws Exception {
        if (recordWriter instanceof NonRecordWriter)
            return;
        int nChannel = recordWriter.getRecordWriter(0).numberOfChannels;
        LOG.info("downstream channel num:{}", nChannel);
        if (nChannel != 1) {
            Map<Integer, Integer> routing = new HashMap<>();
            for (int i = 0; i < nChannel; i++) {
                routing.put((i + 1) % nChannel, i);
            }
            recordWriter.getRecordWriter(0).setRouting(routing);
        }
    }

    public void changeKeyGroupRange(int newIndex) throws IOException, FlinkException {
        int maxparallelism = 128;
        int parallelism = getEnvironment().getTaskInfo().getNumberOfParallelSubtasks();
        if (parallelism == 1)
            return;
        System.out.printf("----------------------------------------\nafter change routing...\n new subindex:%d\n----------------------------------------\n", newIndex);
        this.stateBackend.changeKeyGroupRange(maxparallelism, parallelism, newIndex);
    }

    public void applySnapshot(byte[] bytes, boolean full) throws IOException, FlinkException {
        stateBackend.applySnapshot(bytes, full);
    }

    public void setReplicationManager(ReplicationManager manger) {
        this.replicationManager = manger;
    }

    private boolean isStateful() throws IOException, FlinkException {
        return stateBackend.getStateDescriptor().size() > 0;
    }


    private void applyIncremental(byte[] bytes, int newIndex) throws IOException, FlinkException {
        this.stateBackend.applyIncremental(bytes, newIndex);
    }


    private int getFutureIndex() {
        int parallelism = getEnvironment().getTaskInfo().getNumberOfParallelSubtasks();
        return (getEnvironment().getTaskInfo().getIndexOfThisSubtask() + 1) % parallelism;
    }

    boolean shouldFull() {
        int index = getEnvironment().getTaskInfo().getIndexOfThisSubtask();
        int nIncremental = Integer.parseInt(ProcessLevelConf.getProperty(ProcessLevelConf.NUM_INCREMENTAL));
        return index >= nIncremental;
    }
}
