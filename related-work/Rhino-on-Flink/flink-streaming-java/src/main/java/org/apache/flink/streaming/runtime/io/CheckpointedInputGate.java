...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent>, Closeable {
    ...
    private Optional<BufferOrEvent> handleEvent(BufferOrEvent bufferOrEvent)
        ...
        } else if (bufferOrEvent.getEvent().getClass() == rescaleSignal.class) {
            LOG.info("get rescale signal");
            try {
                this.barrierHandler.freezeChannel(bufferOrEvent.getChannelInfo());
            }catch (Exception e){
                e.printStackTrace();
                LOG.warn("signal error:{}",e.getMessage());
            }
        }
