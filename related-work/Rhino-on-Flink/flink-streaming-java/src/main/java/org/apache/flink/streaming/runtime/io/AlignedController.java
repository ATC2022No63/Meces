...
public class AlignedController implements CheckpointBarrierBehaviourController {
    ...
    //    返回是否全部channel对齐
    @Override
    public boolean freezeChannel(InputChannelInfo info) {
        checkState(
                !blockedChannels.put(info, true),
                "Stream corrupt: Repeated barrier for same checkpoint on input " + info);
        CheckpointableInput input = inputs[info.getGateIdx()];
        input.blockConsumption(info);
        for (Boolean freezed : blockedChannels.values()) {
            if (!freezed)
                return false;
        }
        return true;
