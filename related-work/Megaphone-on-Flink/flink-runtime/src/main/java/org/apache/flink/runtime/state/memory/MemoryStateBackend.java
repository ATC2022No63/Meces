...
import org.apache.flink.runtime.state.mirgration.OperatorStatePartition;
import org.apache.flink.runtime.state.mirgration.StateDescriptor;
...
public class MemoryStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
    ...
    private Map<String, KeyedStateBackend> keyedStateBackends = new HashMap<>();
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(
        ...
        String stateName = jobID + "-" + env.getTaskInfo().getIndexOfThisSubtask();
        this.keyedStateBackends.put(stateName, hkbe);

        return hkbe;
    }

    public BinaryState extractState(int index) throws IOException, FlinkException {
        LOG.info("extract with {} hkbe", keyedStateBackends.size());
        for (Entry<String, KeyedStateBackend> keyedStateBackend : keyedStateBackends
                .entrySet()) {
            OperatorStatePartition[] operatorStatePartitions = keyedStateBackend.getValue().extractState(index);
            return new BinaryState(operatorStatePartitions);
        }
        return null;
    }
    ...
    @Override
    public Set<String> getStateDescriptor() throws FlinkException, IOException {
        return keyedStateBackends.keySet();
    }

    public void insertKG(int index) throws FlinkException, IOException {
        for (KeyedStateBackend backend : keyedStateBackends.values()) {
            backend.insertKG(index);
        }
    }

    @Override
    public void applyState(byte[] bytes) throws FlinkException, IOException {
        DataInputDeserializer dd = new DataInputDeserializer(bytes);
        KeyedStateBackend keyedStateBackend = keyedStateBackends.values().stream().findFirst().get();
        int nstate = dd.readInt();
        for (int i = 0; i < nstate; i++) {
            int len = dd.readInt();
            byte[] buffer = new byte[len];
            dd.read(buffer);
            keyedStateBackend.applyState(buffer);
        }
