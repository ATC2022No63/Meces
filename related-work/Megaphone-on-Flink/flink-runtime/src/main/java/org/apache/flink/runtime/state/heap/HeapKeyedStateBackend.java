...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
    ...
    @Override
    public OperatorStatePartition[] extractState(int index) throws FlinkException, IOException {
        OperatorStatePartition[] states = new OperatorStatePartition[registeredKVStates.size()];
        int n = 0;
        for (Map.Entry<String, StateTable<K, ?, ?>> entry : registeredKVStates.entrySet()) {
            OperatorStatePartition s = entry.getValue().extractState(index);
            states[n++] = s;
        }
        return states;
    }
    ...
    public void insertKG(int index) {
        for (StateTable<K, ?, ?> table : registeredKVStates.values()) {
            table.insertKG(index);
        }
    }

    public void removeKG(int index) {
        for (StateTable<K, ?, ?> table : registeredKVStates.values()) {
            table.removeKG(index);
        }
    }
