...
public abstract class StateTable<K, N, S>
    ...
    protected final Logger log = LoggerFactory.getLogger(StateTable.class);
    ...
    public Map<Integer, StateMap<K, N, S>> routing;
    ...
    public OperatorStatePartition extractState(int index) throws IOException {
        ...
        for (Iterator<StateEntry<K, N, S>> it = tableIter(index); it.hasNext(); ) {
            StateEntry<K, N, S> entry = it.next();
            state.computeIfAbsent(entry.getNamespace(), n -> new HashMap<>()).put(entry.getKey(), entry.getState());
        }

        return new OperatorStatePartition(metaInfo.getName(), index, state, namespaceSerializer, keySerializer, stateSerializer);
    }

    public void applyState(byte[] bytes) throws IOException {
        ...
        dd.readUTF();
        int subindex = dd.readInt();
        int nentries = dd.readInt();
        for (int i = 0; i < nentries; i++) {
            N ns = namespaceSerializer.deserialize(dd);
            K key = keySerializer.deserialize(dd);
            S value = stateSerializer.deserialize(dd);
            put(key, subindex, ns, value);
        }
    }

    public void removeKG(int index) {
    }

    public void insertKG(int index) {
        this.routing.put(index, createStateMap());
        log.info("st insert for kg {}", index);
    }
    ...
    StateMap<K, N, S> getMapForKeyGroup(int keyGroupIndex) {
        if (routing.containsKey(keyGroupIndex)) {
            return routing.get(keyGroupIndex);
        }
    ...
    public void put(K key, int keyGroup, N namespace, S state) {
        ...
        StateMap<K, N, S> stateMap = getMapForKeyGroup(keyGroup);
        if (stateMap == null) {
            log.info("key:{},kg:{},n:{},s:{}", key, keyGroup, namespace, state);
            return;
        }
    ...
    public Iterator<StateEntry<K, N, S>> tableIter(int index) {
        StateMap<K, N, S>[] tables = new StateMap[1];
        tables[0] = keyGroupedStateMaps[indexToOffset(index)];
        this.keyGroupedStateMaps[indexToOffset(index)] = createStateMap();
