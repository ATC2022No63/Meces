...
import org.apache.flink.runtime.state.mirgration.BinaryState;
import org.apache.flink.runtime.state.mirgration.FetchWatcher;
...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
    ...
    public Pubber pubber;
    public int epoch = 0;
    public FetchWatcher fetchWatcher;
    // ------------------------------------------------------------------------
    ...
    protected StreamTask(
        ...
        this.pubber = new Pubber(host, timeout);
        String taskName = environment.getTaskInfo().getTaskName();
        if (taskName.equals(ProcessConf.getProperty(ProcessConf.RESCALE_TASK_NAME))) {
            fetchWatcher = new FetchWatcher(environment.getTaskInfo().getIndexOfThisSubtask(), this);
            String topic = ProcessConf.getProperty(ProcessConf.FETCH_TOPIC);
            fetchWatcher.watch(topic);
        }
    }
    ...
    public void ackSignal() {
        ...
        LOG.info("{} in subtask {}, epoch {}", prefix, index, epoch++);
        pubber.send(topic, prefix + ":" + epoch + ":" + index);
        return;
    }

    public void broadcastBatchSignal(EndOfBatchEvent endOfBatchEvent) {
        try {
            if (recordWriter instanceof NonRecordWriter) {
                ackSignal();
            } else {
                int index = endOfBatchEvent.rescaleIndex;
                if (index != -1) {
                    String thisName = getEnvironment().getTaskInfo().getTaskName();
                    if (thisName.equals(ProcessConf.getProperty(ProcessConf.UPSTREAM_TASK_NAME))) {
                        byte[] bytes = doMigration(index);
                        if (bytes != null)
                            endOfBatchEvent.setBytes(bytes);
                    } else if (thisName.equals(ProcessConf.getProperty(ProcessConf.RESCALE_TASK_NAME)))
                        modifyState(endOfBatchEvent);
                }
                recordWriter.getRecordWriter(0).broadcastEvent(endOfBatchEvent, true);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void broadcastMigrationSignal() {
    }

    public byte[] doMigration(int index) throws Exception {
        TaskInfo ti = getEnvironment().getTaskInfo();
        int parallelism = ti.getNumberOfParallelSubtasks();
        int thisindex = ti.getIndexOfThisSubtask();
        recordWriter.getRecordWriter(0).setRouting(index);
        LOG.info("change routing with index:{}", index);
        if (index % parallelism == thisindex) {
            byte[] bytes = fetchState(index);
            LOG.info("get bytes:{}", bytes.length);
            return bytes;
        }
        return null;
    }
    ...
    public void modifyState(EndOfBatchEvent endOfBatchEvent) throws IOException, FlinkException {
        int kg = endOfBatchEvent.rescaleIndex;
        if (kg == -1)
            return;
        TaskInfo ti = getEnvironment().getTaskInfo();
        int parallelism = ti.getNumberOfParallelSubtasks();
        int newIndex = KeyGroupRangeAssignment.toNewIndex(kg, 128, parallelism);
        if (newIndex != ti.getIndexOfThisSubtask())
            return;
        insertKG(kg);
        applyState(endOfBatchEvent.bytes);
        LOG.info("change state for {}-{},{} subtask", ti.getTaskName(), ti.getIndexOfThisSubtask(), endOfBatchEvent.rescaleIndex);
    }

    public BinaryState getState(int index) throws IOException, FlinkException {
        if (stateBackend == null)
            return null;
        return stateBackend.extractState(index);
    }


    public void insertKG(int index) throws IOException, FlinkException {
        this.stateBackend.insertKG(index);
    }

    public void removeKG(int index) throws IOException, FlinkException {
        this.stateBackend.removeKG(index);
    }

    public void triggerSignal() {
    }

    public void applyState(byte[] bytes) {
        try {
            this.stateBackend.applyState(bytes);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public int getStateDescriptor() throws IOException, FlinkException {
        return stateBackend.getStateDescriptor().size();
    }
    ...
    public byte[] fetchState(int subindex) throws IOException {
        int index = getEnvironment().getTaskInfo().getIndexOfThisSubtask();
        int port = Integer.valueOf(ProcessConf.getProperty(ProcessConf.MIGRATION_PORT))
                + index;
        String host = InetAddress.getLocalHost().getHostName();
        String msg = String.format("%s:%s:%s:%s", "fetch1", subindex,
                host, port);
        String pubTopic = ProcessConf.getProperty(ProcessConf.FETCH_TOPIC);
        pubber.send(pubTopic, msg);
    ...
    @Override
    public void sendState(int subindex, String addr, int port) throws FlinkException, IOException {
        LOG.info("{} start send {}",getEnvironment().getTaskInfo().getIndexOfThisSubtask(),subindex);
        byte[] bytes = getState(subindex).toBytes();
        ...
        os.write(bytes);
        os.close();
        LOG.info("{} end send {}",getEnvironment().getTaskInfo().getIndexOfThisSubtask(),subindex);
    }
