...
public class CheckpointBarrierTracker extends CheckpointBarrierHandler {
    ...
    public CheckpointBarrierTracker(int totalNumberOfInputChannels, AbstractInvokable toNotifyOnCheckpoint) {
        ...
        this.getSignal = new boolean[totalNumberOfInputChannels];
    }
    ...
    public void receiveSignal(BufferOrEvent bufferOrEvent) {
        ...
        getSignal[info.getGateIdx()] = true;
        for (boolean got : getSignal) {
            if (!got)
                return;
        }
        ...
        try {
            ...
            toNotifyOnCheckpoint.broadcastBatchSignal(endOfBatchEvent);
