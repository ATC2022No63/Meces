...
public class SingleCheckpointBarrierHandler extends CheckpointBarrierHandler {
    ...
    public void receiveSignal(BufferOrEvent bufferOrEvent) {
        if (!(controller instanceof AlignedController)) {
            LOG.warn("not AlignedController");
            return;
        }
        InputChannelInfo info = bufferOrEvent.getChannelInfo();
        boolean aligned = controller.receiveSignal(info);
        try {
            if (aligned) {
                EndOfBatchEvent endOfBatchEvent = (EndOfBatchEvent) bufferOrEvent.getEvent();
                this.toNotifyOnCheckpoint.broadcastBatchSignal(endOfBatchEvent);
            }
